...
resource "flexibleengine_lb_listener_v2" "listeners" {
  ...
  depends_on = [
    flexibleengine_elb_certificate.cert
  ]
}
...
resource "flexibleengine_lb_pool_v2" "pools" {
  ...
  /*
  description = element(var.pools.*.desc, count.index) == "" ? null : element(var.pools.*.desc, count.index)
  admin_state_up = element(var.pools.*.admin_state_up, count.index)
  persistence = {
    type        = element(var.pools.*.persistence.type, count.index) == "" ? null : element(var.pools.*.desc, count.index)
    cookie_name = element(var.pools.*.persistence.cookie_name, count.index) == "" ? null : element(var.pools.*.desc, count.index)
    timeout     = element(var.pools.*.persistence.timeout, count.index)
  }
  */
}
...
resource "flexibleengine_lb_member_v2" "members" {
  ...
  /*
  weight = element(var.pools.*.weight, count.index) == 0 ? null : element(var.pools.*.weight, count.index)
  admin_state_up = element(var.pools.*.admin_state_up, count.index) 
  */
...
resource "flexibleengine_lb_monitor_v2" "monitor" {
  ...
  /*
  url_path = var.monitors[count.index].url_path == "" ? null : element(var.monitors[count.index].url_path, count.index)
  http_method = var.monitors[count.index].http_method == "" ? null : element(var.monitors[count.index].http_method, count.index)
  expected_codes = var.monitors[count.index].expected_codes == "" ? null : element(var.monitors[count.index].expected_codes, count.index)
 
  admin_state_up = var.monitors[count.index].admin_state_up
  */
...
resource "flexibleengine_lb_whitelist_v2" "whitelists" {
  ...
  whitelist        = var.listeners_whitelist[count.index].whitelist == "" ? null : element(var.listeners_whitelist[count.index].whitelist, count.index)
}
...
resource "flexibleengine_lb_l7policy_v2" "l7policies" {
  ...
  description          = var.l7policies[count.index].description  == "" ? null : element(var.l7policies[count.index].description, count.index)

  //admin_state_up = var.l7policies[count.index].admin_state_up
}
...
resource "flexibleengine_lb_l7rule_v2" "l7rules" {
  ...
  key          = var.l7policies_rules[count.index].key == "" ? null : element(var.l7policies_rules[count.index].key, count.index)
  
  //admin_state_up = var.l7policies_rules[count.index].admin_state_up
}
